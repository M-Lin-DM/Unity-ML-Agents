...
//MRL added below get statement
        public float[] M_Observations_get
            {
        ...
        public int Write(WriteAdapter adapter)
        {
            using (TimerStack.Instance.Scoped("RayPerceptionSensor.Perceive"))
            {
                ...
                for (var rayIndex = 0; rayIndex < numRays; rayIndex++)
                {
                    ...
                    rayOutput.ToFloatArray(numDetectableTags, rayIndex, m_Observations); //third arg is the buffer of observations that is written to by this function
                }
                // Finally, add the observations to the WriteAdapter
                adapter.AddRange(m_Observations);
                //MRL added writetocsv
                adapter.WriteToCSV(m_Observations);
            }
            return m_Observations.Length;
        }

        // public void Write2CSV(WriteAdapter adapter)
        // {
